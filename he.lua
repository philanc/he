...
content:
  ...
  -- iterator functions (coroutine-based iterator objects)
  items         generate the items of a list
  count         generate sequences of numberss
  take          limit the number of item returned by an iterator
  first         return the first value generated by an iterator
  tolist        return a list with all the items generated by an iterator
  filter        filter the items generated by another iterator
  matching      filter the items matching a pattern
  notmatching   filter the items not matching a pattern
  map           apply a function to the items generated by another iterator
  fread         iterate on lines or fixed size blocks from a file
  records       iterate on records separated by a pattern
  lines         iterate on lines 
...
local iter = class() -- see methods further down
...
function list.items(lst)
	-- iterator over all lst elements, eg: for x in lst:items() do ... end
	-- (return an iter object, so it can be chained with other iterators)
	return iter.items(lst)
end
...
function iter:__call(...)
	-- iter objects are callable. This is what allows a simple syntax:
	--    for x in it do ... end  -- where 'it' is an iter
	return self.f(...)
end
...
function iter.items(t)
	-- create a source iterator
	-- return elements of a sequence t (same iteration as ipairs, but it )
	return iter.cowrap(function()
...
function iter.fread(fh, bsize)
	-- iterate on an openned file content
	-- fh is the file handle
	-- bsize is the block size (iterator reads bsize bytes at a time)
	-- bsize is optional. If not provided, the iterator return one 
	-- line at a time.
	-- eg.  fh = io.open("fname")
	--      for l in iter.fread(fh) do print(l) end
	--      fh:close()
	bsize = bsize or "l" -- default is to read lines (w/o line terminator)
	return iter.cowrap(function()
		while true do
			local x = fh:read(bsize) -- read block or line 
			if x then yield(x) else return nil end
		end
	end)
end --fread()

function iter.records(it, pat)
	-- iterate on records separated by a pattern
	if it.err then return nil, it.msg end
	return iter.cowrap(function()
		local i, j, x, rest
		local i0 = 1
		x = it() -- get first block
...
--~ 			print(i, j, 'i0', i0, #x, x)
			if i then 
				...
				-- pat not found. look for more
				rest = x:sub(i0)
				x = it() -- get next block
				if not x and #rest == 0 then 
					return nil
				elseif not x then 
					yield(rest)
					return nil
...
function iter.lines(it)
	-- iterate on the lines contained in a block iterator
	return iter.records(it, "\r?\n")
end
