...
local function parseitext(itxt)
	...
	itxt = he.stripeol(he.strip(itxt:gsub("\r", ""))) -- clean itxt 
	itxt =itxt .. '\n' -- simplify detection of '===' at end of text...
...
local function makeitext(itemlist, header, toc)
	-- build a text from a list of items, return the text
	-- header (optional): text inserted before the first item
	-- toc (optional): a boolean. if true, a TOC is inserted 
	--    between header and first item
	local sep = "\n\n\n"
	local txtl = itemlist:map(itemtos)
	if toc then table.insert(txtl, 1, maketoc(itemlist)) end
	if header then table.insert(txtl, 1, header) end
