...
--180821 adapt to luazen-0.11 => v0.6
...
local pcrypt_VERSION = "pcrypt 0.6"
...
local encrypt = lz.morus_encrypt --  (k, n, m, ninc, ad) => c
local decrypt = lz.morus_decrypt --  (k, n, c, ninc, adlen) => m | nil,err
...
local function key_exchange(ask, bpk)
	local sec = lz.x25519_shared_secret(ask, bpk)
	local k = lz.blake2b(sec, 32)
	return k
end
...
function encrypt_stream(k, fhi, fho, pkflag)
	...
	if pkflag then
		pk = k
		rpk, rsk = keypair()
		nonce32 = rpk  -- use the random pk as the nonce
		k = key_exchange(rsk, pk) -- get the session key
	else
		nonce32 = lz.randombytes(32)
	end
	if noncelen < 32 then
		nonce = nonce32:sub(1, noncelen)
	else
		nonce = nonce32
	end
	local ninc = 0
	...
	while not eof do
		...
		eof = (#block < rdlen)
		local cblock = encrypt(k, nonce, block, ninc, aad)
		ninc = ninc + 1
		fho:write(cblock)
	end--while
end--encrypt_stream()
...
function decrypt_stream(k, fhi, fho, pkflag)
	...
	while not eof do
		...
		local pblock, msg = decrypt(k, nonce, block, ninc, aadlen)
		ninc = ninc + 1
